...
def dump(pkt):
    # the last 4 bytes of the packet is the CRC32 of the preceeding bytes. 
    # Calculate the CRC of the packet, extract the encoded CRC, and compare
    calc_crc = binascii.crc32(pkt[0:len(pkt)-4])
    pkt_crc = int.from_bytes(pkt[len(pkt)-4:], "little")
    if calc_crc != pkt_crc:
        logging.error("calculated CRC {} does not match packet CRC {}".format(hex(calc_crc), hex(pkt_crc)))
